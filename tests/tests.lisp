...
(define-test parse-combinator-input-test
  (is term-equal
      (get-combinator 'W**)
      (parse-combinator-input "W**"))
  (is term-equal
  ...
  (is term-equal
      (make-combinator-application
       ...
       (make-combinator-application
        (make-combinator-variable #\x)
        (make-combinator-variable #\z)))
      (parse-combinator-input "x y(z w y)(x z)"))
  (is term-equal
      (make-combinator-application
       (make-combinator-application
        (make-combinator-application
        ...
        (make-combinator-application
         ...
         (get-combinator 'L)))
       (get-combinator 'T))
      (parse-combinator-input "SBCW(ML)T")))

(define-test parse-lambda-input-test
  (is term-equal
      (make-lambda-application
       (make-lambda-variable #\x)
       (make-lambda-variable #\y))
      (parse-lambda-input "x y"))
  (is term-equal
      (make-lambda-application
       (make-lambda-application
      ...
      (parse-lambda-input "v(w w)u"))
  (is term-equal
      (make-lambda-abstraction
       (make-lambda-variable #\x)
       (make-lambda-variable #\x))
      (parse-lambda-input "λx.x"))
  (is term-equal
      (make-lambda-application
       (make-lambda-abstraction
        (make-lambda-variable #\x)
  ...
  (is term-equal
      ...
      (parse-lambda-input "(λx.x)(λf y.f y y)m")))

(define-test lambda-equality-test
  :depends-on (parse-lambda-input-test)
  (macrolet ((expect-lambda-equal (comp term1 term2)
               `(,comp (term-equal (parse-lambda-input ,term1)
                                   (parse-lambda-input ,term2)))))
...
(define-test combinator-reduction-test
  :depends-on (parse-combinator-input-test)
  (macrolet ((is-reduced (expected term)
               `(is term-equal
                    (parse-combinator-input ,expected)
                    (reduce-term (parse-combinator-input ,term)))))
...
(define-test lambda->ski-test
  :depends-on (parse-combinator-input-test parse-lambda-input-test)
  (is term-equal
      (parse-combinator-input "S(K(SI))K")
      (lambda->ski (parse-lambda-input "λx y.y x")))
  (is term-equal
      (make-combinator-application
       (make-combinator-variable #\b)
       (make-combinator-variable #\a))
      (reduce-term (lambda->ski (parse-lambda-input "(λx y.y x)a b"))))
  (is term-equal
      (get-combinator 'K)
      (reduce-term (lambda->ski (parse-lambda-input "(λn.n(λx.λx y.y)(λx y.x))λf x.x"))))
  (is term-equal
      (make-combinator-application
       (get-combinator 'K)
       (get-combinator 'I))
      (reduce-term (lambda->ski (parse-lambda-input "(λn.n(λx.λx y.y)(λx y.x))λf x.f x")))))

(define-test lambda->sk-test
  :depends-on (parse-combinator-input-test parse-lambda-input-test)
  (is term-equal
      (parse-combinator-input "S(K(S(SKK)))K")
      (lambda->sk (parse-lambda-input "λx y.y x")))
  (is term-equal
      (make-combinator-application
       (make-combinator-variable #\b)
       (make-combinator-variable #\a))
      (reduce-term (lambda->sk (parse-lambda-input "(λx y.y x)a b"))))
  (is term-equal
      (get-combinator 'K)
      (reduce-term (lambda->sk (parse-lambda-input "(λn.n(λx.λx y.y)(λx y.x))λf x.x"))))
  (is term-equal
      (parse-combinator-input "K(SKK)")
      (reduce-term (lambda->sk (parse-lambda-input "(λn.n(λx.λx y.y)(λx y.x))λf x.f x")))))

(define-test combinator->lambda-test
  :depends-on (combinator-reduction-test parse-lambda-input-test lambda-equality-test)
  (is term-equal
      (combinator->lambda (get-combinator 'M))
      (parse-lambda-input "λx.x x"))
  (is term-equal
      (combinator->lambda (get-combinator 'B))
      (parse-lambda-input "λx y z.x(y z)"))
  (is term-equal
      (combinator->lambda (get-combinator 'L))
      (parse-lambda-input "λx y.x(y y)"))
  (is term-equal
      (combinator->lambda (get-combinator 'W))
      (parse-lambda-input "λx y.x y y"))
  (is term-equal
      (combinator->lambda (get-combinator 'K))
      (parse-lambda-input "λx y.x"))
  (is term-equal
      (combinator->lambda (get-combinator 'I))
      (parse-lambda-input "λx.x"))
  (is term-equal
      (combinator->lambda (get-combinator 'S))
      (parse-lambda-input "λx y z.x z(y z)")))
...
(define-test lambda-reduction-test
  ...
  (macrolet ((is-reduced (expected term)
               `(is term-equal
                    (parse-lambda-input ,expected)
                    (reduce-term (parse-lambda-input ,term)))))
    (is-reduced "x" "x")
    (is-reduced "y" "(λx.x)y")
...
(define-test church-numerals-test
  :depends-on (parse-lambda-input-test
               lambda-equality-test)
  (is term-equal (parse-lambda-input "λf x.x") (natural->church 0))
  (is term-equal (parse-lambda-input "λf x.f x") (natural->church 1))
  (is term-equal (parse-lambda-input "λf x.f(f x)") (natural->church 2))
  (is term-equal (parse-lambda-input "λf x.f(f(f x))") (natural->church 3))
  (is term-equal (parse-lambda-input "λf x.f(f(f(f x)))") (natural->church 4))
  (is term-equal (parse-lambda-input "λf x.f(f(f(f(f(f(f x))))))") (natural->church 7))
  (is term-equal (parse-lambda-input "λf x.f(f(f(f(f(f(f(f(f(f(f(f x)))))))))))") (natural->church 12))
  (is = 0 (church->natural (parse-lambda-input "λf x.x")))
  (is = 1 (church->natural (parse-lambda-input "λf x.f x")))
  (is = 3 (church->natural (parse-lambda-input "λf x.f(f(f x))")))
  (is = 8 (church->natural (parse-lambda-input "λf x.f(f(f(f(f(f(f(f x)))))))")))
  (is = 11 (church->natural (parse-lambda-input "λf x.f(f(f(f(f(f(f(f(f(f(f x))))))))))"))))

(define-test barendregt-numerals-test
  :depends-on (parse-combinator-input-test)
  (is term-equal (parse-combinator-input "I") (natural->barendregt 0))
  (is term-equal (parse-combinator-input "V(KI)I") (natural->barendregt 1))
  (is term-equal (parse-combinator-input "V(KI)(V(KI)I)") (natural->barendregt 2))
  (is term-equal (parse-combinator-input "V(KI)(V(KI)(V(KI)I))") (natural->barendregt 3))
  (is term-equal (parse-combinator-input "V(KI)(V(KI)(V(KI)(V(KI)I)))") (natural->barendregt 4))
  (is = 0 (barendregt->natural (parse-combinator-input "I")))
  (is = 1 (barendregt->natural (parse-combinator-input "V(KI)I")))
  (is = 2 (barendregt->natural (parse-combinator-input "V(KI)(V(KI)I)")))
  (is = 3 (barendregt->natural (parse-combinator-input "V(KI)(V(KI)(V(KI)I))")))
  (is = 4 (barendregt->natural (parse-combinator-input "V(KI)(V(KI)(V(KI)(V(KI)I)))"))))

(define-test goedelization-test
  :depends-on (parse-combinator-input-test)
  (macrolet ((is-goedel (n term)
               `(progn
                  (is = ,n (sk->goedel (parse-combinator-input ,term)))
                  (is term-equal (parse-combinator-input ,term) (goedel->sk ,n)))))
...
(define-test lambda-programs-test
  ...
  (macrolet ((lambda-program-result (path)
    ...
    (is term-equal (parse-lambda-input "λx y.y") (lambda-program-result #p"basic.lam"))))
